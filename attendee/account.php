...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['about_me'])) {
    ...
    $update = mysqli_query($con, "UPDATE usertable SET about='$about_me' WHERE id='$user_id'");
    if ($update) {
...
<body>
    ...
    <div class="container">
        ...
        <div class="row p-3 py-4"
            style="border-radius: 6px; background: linear-gradient(rgba(26, 34, 40, 1), rgba(0, 0, 0, 0.7)) !important;">
            ...
            <div class="col-md-9 ps-4">
                <div class="row">
                    <div class="account-details col-12">
                        ...
                        $user = null;
                        if ($user_id) {
                            $result = mysqli_query($con, "SELECT name, role, created_at, username, email, about FROM usertable WHERE id = '$user_id' LIMIT 1");
                                ...
                                <div class="card shadow rounded-4 border-0 p-5 w-100"
                                    ...
                                    <div class="row mb-4 w-100" style="max-width:700px;margin:0 auto;">
                                        ...
                                        <div class="col-md-6 col-12 mb-3 mb-md-0 text-end">
                                            <label class="form-label fw-semibold text-muted text-end w-100">Username</label>
                                            <div class="fs-5 text-end"><?php echo htmlspecialchars($user['username']); ?>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row mb-2 w-100" style="max-width:700px;margin:0 auto;">
                                        <label class="form-label fw-semibold text-muted">About Me</label>
                                        <form method="POST" id="aboutMeForm" class="d-flex align-items-center gap-2"
                                            style="max-width: 100%;">
                                            <input type="text" name="about_me" id="aboutMeInput" maxlength="255"
                                                class="form-control" style="width:100%;"
                                                value="<?php echo htmlspecialchars($user['about'] ?? ''); ?>" <?php if (!isset($_GET['edit_about']))
                                                         echo 'readonly'; ?>>
                                            ...
                                            <?php else: ?>
                                                ...
                                                <button type="submit" class="btn btn-outline-success btn-sm about-btn-compact"
                                                    id="confirmAboutBtn"><i class="fa fa-check"></i>Confirm</button>
                                            <?php endif; ?>
                                        </form>
                                    </div>
                                    <script>
                                        ...
                                        if (aboutMeInput && !aboutMeInput.hasAttribute('readonly')) {
                                            aboutMeInput.focus();
                                        }
                                    </script>
